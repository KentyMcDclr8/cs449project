...
## The major changes that were made are highlighted by comments!
import json
import os
import sys
import numpy as np
...
class Seq2SeqAgent(BaseAgent):
    ...
    def _sort_batch(self, obs):
        ...
        keys_list=[]
        for ob in obs:
           keys = [key for key in ob.keys() if key.startswith('instr_encoding_')]
        ...
        for ob in obs:
            ...
            current_seq_tensors = []
            for item in keys:
                current_seq_tensor = np.array(ob[f'instr_encoding_{num}'])
                current_seq_tensors.append(current_seq_tensor)
                num += 1
            seq_tensor.append(current_seq_tensors) 
    ...
    def rollout(self):
        ...
        ## The Sequence is passed at once completely for all the instructions! So, seq contains all the instructions
        ## are there in ONE batch size. So, for sequential thing, plan your stuff accordingly. Maybe, reducing the batch
        ## size to one might help Insha Allah!

...
class R2RBatch():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None):
        ...
        for item in load_datasets(splits):
            ...
            for j,instr in enumerate(item['instructions']):
                ...
                new_item['instructions'] = perform_sentence_segmentation(instr)
                counter=0
                if tokenizer:
                    for inst in new_item['instructions']:
                        new_item[f'instr_encoding_{counter}'] = tokenizer.encode_sentence(inst) 
                        counter+=1
    ...
    def _get_obs(self):
        ...
        for i,(feature,state) in enumerate(self.env.getStates()):
            ...
            num=0
            for num in range(15):
                if f'instr_encoding_{num}' in item:
                    obs[-1][f'instr_encoding_{num}'] = item[f'instr_encoding_{num}']
        return obs
